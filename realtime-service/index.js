...
const validateInternalRequest = (req, res, next) => {
  ...
  if (!INTERNAL_API_KEY) {
    ...
    return res
      .status(500)
      .json({ error: "Internal server configuration missing." });
  }
...
app.post("/internal/notify", validateInternalRequest, (req, res) => {
  ...
  if (!roomId || typeof roomId !== "string" || !roomId.trim()) {
    return res
      .status(400)
      .json({ error: "roomId es obligatorio y debe ser un string no vacío." });
  }

  if (!event || typeof event !== "string" || !event.trim()) {
    return res
      .status(400)
      .json({ error: "event es obligatorio y debe ser un string no vacío." });
  }
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "payload es obligatorio y debe ser un objeto válido." });
  }
  ...
  socket.on("join_room", ({ roomId, role }) => {
    if (!roomId || typeof roomId !== "string" || !roomId.trim()) {
      ...
      return socket.emit("error", {
        message: "roomId es obligatorio para unirse a una sala.",
      });
    }
    ...
    log(
      ...
      `socketId=${socket.id} roomId=${roomId} role=${socket.data.role}`,
    );
  });
  ...
  socket.on("request_sync", ({ roomId }) => {
    if (!roomId || typeof roomId !== "string" || !roomId.trim()) {
      ...
      return socket.emit("sync_error", {
        message: "roomId es obligatorio para request_sync.",
      });
    }
    ...
    if (!hostId) {
      log(
        "Host no encontrado para request_sync",
        `socketId=${socket.id} roomId=${roomId}`,
      );
      return socket.emit("sync_error", {
        message: "No hay host registrado para esta sala.",
      });
    }

    if (hostId === socket.id) {
      log(
        "request_sync ignorado porque el host solicitó sync sobre sí mismo",
        `socketId=${socket.id} roomId=${roomId}`,
      );
      return;
    }
    ...
    log(
      "Solicitud de sync enviada al host",
      `requesterId=${socket.id} hostId=${hostId} roomId=${roomId}`,
    );
  ...
  socket.on("disconnect", (reason) => {
    ...
    rooms.forEach((roomId) => {
      if (roomHosts.get(roomId) === socket.id) {
        ...
        log(
          "Host desconectado y eliminado del mapeo",
          `roomId=${roomId} hostId=${socket.id}`,
        );
      }
    });
    ...
    log(
      "Socket desconectado",
      `socketId=${socket.id} rooms=${rooms.join(", ") || "ninguna"} reason=${reason}`,
    );
  });
});
